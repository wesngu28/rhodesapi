...
export const getStaticInformation = async (
  url: string,
  imgArr?: Array<{ name: string; link: string; line?: string }>
) => {
  try {
    ...
    const cells = rangeBox.map((currRange, i) => {
      ...
      const range = Array.from(currCells).map(cell => {
        let blocks = Array.from(cell.querySelectorAll('span')).map(block =>
          block.classList.contains('empty-box') ? 'attackable' : block.classList.contains('fill-box') ? 'unit' : 'null'
        )
        return blocks
      })
      return {
    ...
    const [primaryClass, secondaryClass] = classArr[1]
      ? classArr[1].includes('/')
        ? classArr[1].split(' / ')
        : [classArr[1]]
      : []
    ...
    const trust: Array<{ name: string; value: string }> = operator
      .querySelectorAll('.trust-cell .potential-list')
      .map(cell => {
        return {
          name: checkForExistence(cell.querySelector('.potential-icon')),
          value: checkForExistence(cell.querySelector('.potential-title')),
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      modulo.forEach(modulos => {
        ...
        const moduleAttributeNames = modulos
          ...
          .slice(2)
          .map(moduleAttributeName => checkForExistence(moduleAttributeName))
        const moduleAttributeValues = modulos
          .querySelectorAll('.module-row-3 > table tr > td')
          .map(moduleAttributeValue => checkForExistence(moduleAttributeValue))
        moduleAttributeNames.forEach(
          (moduleAttributeName, i) => (moduleAttribute[moduleAttributeName] = moduleAttributeValues[i])
        )
        ...
        const unlockCriteria = [
          ...new Set(
            modulos
              .querySelectorAll('.accordion-custom-content:last-child > table:nth-child(2) > tr')
              .map((criteria, i) => {
        ...
        if (existingModuleIndex !== -1) {
          ...
          modules.push({
            ...
            availability: checkForExistence(modulos.querySelector('.module-availability')).includes('Availability: na')
              ? 'Global'
              : 'CN',
            levels: [moduleData],
          })
    ...
    const base = operator.querySelectorAll('.building-buff-cell').map(el => {
      ...
      const level = el.querySelector('.level-cell img[src]')?.getAttribute('src')?.[41]
        ? `Elite ${el.querySelector('.level-cell img[src]')!.getAttribute('src')![41]}`
        : ''
    ...
    const infoKeys = operator
      .querySelectorAll('.profile-info-table > table:nth-child(2) th, .profile-info-table > table:nth-child(4) th')
      .map(th => checkForExistence(th).replaceAll(' ', '_').toLowerCase())
    const infoBody = operator
      .querySelectorAll('.profile-info-table > table:nth-child(2) td, .profile-info-table > table:nth-child(4) td')
      .map(td => checkForExistence(td))
    ...
    const affiliations: string[] = operator
      .querySelectorAll('.group-name > a')
      ?.map(affiliation => checkForExistence(affiliation))

    const voiceLines: { [key: string]: string } = {}
    const accordionContent = operator.querySelector('.voice-lines-table')
    const voiceLineConditions = (accordionContent?.querySelectorAll('th') || []).map(th =>
      checkForExistence(th).replaceAll(' ', '_').toLowerCase()
    )
    ...
    for (let i = 0; i < lbs.length; i++) {
      const currentWikiLink = imgLinkList[i]
      if (imgArr && imgArr[i] && imgArr[i].link !== currentWikiLink) {
        console.log(`Image link updated for ${operatorName} - ${i === 0 ? 'Base' : `E${i}`}`)
        operatorArt.push({
          name: `${i === 0 ? 'Base' : `E${i}`}`,
          link: currentWikiLink,
        })
      } else {
        operatorArt.push({
          name: `${i === 0 ? 'Base' : `E${i}`}`,
    ...
    if (imgLinkList.length > 0) {
      for (let i = 0; i < imgLinkList.length; i++) {
        ...
        const adjustedIndex = i + lbs.length

        if (imgArr && imgArr[adjustedIndex] && imgArr[adjustedIndex].link !== skinUrl) {
          console.log(`Skin link updated for ${operatorName} - ${name}`)
          operatorArt.push({ name, link: skinUrl, line })
        } else {
          operatorArt.push({ name, link: skinUrl, line })
        }
      }
...
async function handleRelativeImageLinks(imgLink: string): Promise<Array<string>> {
  ...
  return [
    name,
    line,
    skinUrl?.getAttribute('href')?.includes('https://gamepress.gg/')
      ? skinUrl.getAttribute('href')!
      : `https://gamepress.gg${skinUrl?.getAttribute('href')}`,
  ]
}
